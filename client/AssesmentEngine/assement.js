...
function generateQuestion() {
    ...
    // If all questions have been asked, don't repeat - keep showing the completion
    if (remainingQuestions.length === 0) {
        return; // Stop asking more questions
    }
    ...
    // Track this question
    const originalIndex = levelQuestions.indexOf(selectedQuestion);
...
function submitAnswer() {
    // Check if assessment is complete (15 questions)
...
function showCompletionMessage() {
    ...
    inputAreaEl.innerHTML = '';
    feedbackEl.textContent = `Congratulations! You answered ${correctAnswers} out of 15 questions correctly!`;
    feedbackEl.className = 'feedback correct';
    ...
    personalizationEl.innerHTML = `
        <div style="margin-top: 20px; padding: 20px; background: linear-gradient(135deg, #00d4ff 0%, #0099cc 100%); color: white; border-radius: 10px; text-align: center; font-size: 18px; font-weight: 600;">
            Your Final Score: ${correctAnswers}/15
            <br><br>
